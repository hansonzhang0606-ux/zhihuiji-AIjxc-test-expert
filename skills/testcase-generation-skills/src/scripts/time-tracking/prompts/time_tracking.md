...
> 1. **完成通报 + 产出物展示**
> 2. **立即触发时间数据收集**（见本文件第四节）—— **禁止先说「下一步怎么走」或展示「进入下一环节」等选项**
> 3. **解析 + 二次确认 + 保存**（`record_time_saved.py`）
> 4. **确认记录完成**后，方可展示下一步选项或等待用户指令
>
> 用户拒绝填写时**最多追问 2 次**，仍拒绝则记录 `time_saved_hours=0` 并标注「用户未反馈」——但**仍属于「完成时间收集环节」**，之后才允许展示下一步选项。
>
...
> 时机（v5.4 立即触发+阻塞）：**环节 approve / 交付确认之后、进入下一环节之前**。
> ⛔ **立即触发**：产出物交付后**必须立即**触发时间收集，**禁止先展示「下一步怎么走 / 进入下一环节」等选项**；
> 必须先完成「通报 → 询问 → 解析 → 二次确认 → 写本地 JSONL」全流程，**确认记录完成后**才允许展示下一步选项。
> 时间追踪不阻塞已 approve 的环节本身，但**未完成收集不得推进下一环节**。
...
13. **立即触发+阻塞（v5.4）**：每个追踪环节产出交付后，**必须先完成时间收集**（通报 → 询问 → 解析 → 二次确认 → 写本地 JSONL），**确认记录完成后**才允许展示下一步选项；**禁止在收齐时间数据前展示下一步**、禁止把时间询问合并到下一步对话、禁止跳过时间询问。
